...
\begin{definition}
  ...
  Fix a function $f$.
  The \defterm{derivative function of $f$} (or \defterm{derivative of $f$} for short) is the function $f'$ (read \enquote{$f$ prime}) given by
  \begin{equation}
    \label{eq:deriv.func}
    f'(x) = \lim_{\Delta x \to 0} \frac{f \pbrac{x + \Delta x} - f \pbrac{x}}{\Delta x}
  \end{equation}
  wherever the limit exists.
  If $f'$ exists on some region $I$, we say that $f$ is \defterm{differentiable on $I$}; if $f'$ exists on the whole domain of $f$, we say that $f$ is \defterm{differentiable}.
...
\begin{definition}
  ...
  Continuing this process, if we can take $n$ successive derivatives of $f$ for some integer $n \geq 1$, the resulting function is the \defterm{$n$th derivative} of $f$, denoted $f^{(n)}$ or $\D[n]{}{x} f$.
  In this case, $f$ is \defterm{$n$ times differentiable}.
  If $f$ is $n$ times differentiable for all integers $n$, $f$ is \defterm{infinitely differentiable} or \defterm{smooth}.
\end{definition}
