...
\begin{definition}
  ...
  Then $F$ is an \defterm{antiderivative of $f$ on $I$} if $\D{F}{x} = f \pbrac{x}$ on $I$.
\end{definition}
...
\begin{definition}
  ...
  Then the \defterm{general antiderivative of $f$} is the one-parameter family of functions $\AD{}{x} f(x) = F(x) + C$ where $C$ takes values from $\RR$.
\end{definition}
